...
const getPrivatCurrency = (chatId, msg) => {
  ...
  axios.get(endpoint).then((resp) => {
    ...
    const privatCurr = `${date.toLocaleString()}.\nPrivatbank actual currency is: \nUSD: Buy:${parseFloat(
      currUsd.buy
    ).toFixed(2)}; Sell: ${parseFloat(currUsd.sale).toFixed(2)} \nEUR: Buy:${parseFloat(currEur.buy).toFixed(
      2
    )}; Sell:${parseFloat(currEur.sale).toFixed(2)}`;
  ...
  setInterval(() => {
    axios
      ...
      .then((resp) => {
        ...
        fs.writeFileSync("mono.json", JSON.stringify(monoCurr, null, " "));
      })
      .catch((err) => {
...
const weatherTemplate = (city, elem) => {
  let weather = `\t${elem.dt_txt}:
    +${Math.trunc(elem.main.temp)}°C, ощущается как: +${Math.trunc(elem.main.feels_like)}°C, ${
